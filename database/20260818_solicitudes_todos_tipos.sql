-- Ejecutar una sola vez en Supabase después del esquema canónico v3.
-- Habilita la creación transaccional de todos los tipos usados por la intranet.
begin;
...
begin
    ...
    if p_clave_tipo not in ('HORAS_EXTRA','PERMISO_CON_GOCE','PERMISO_SIN_GOCE','AUSENCIA_TEMPORAL','MODIFICACION_TURNO','VACACIONES') then
    ...
    if v_jefe.id=v_u.id then raise exception 'AUTOAPROBACION_NO_PERMITIDA: el solicitante no puede ser su propio responsable.'; end if;

    v_prefijo := case p_clave_tipo when 'HORAS_EXTRA' then 'HE' when 'PERMISO_CON_GOCE' then 'PCG' when 'PERMISO_SIN_GOCE' then 'PSG' when 'AUSENCIA_TEMPORAL' then 'AT' when 'MODIFICACION_TURNO' then 'MT' else 'VAC' end;
    v_folio := format('%s-%s-%s',v_prefijo,to_char(current_date,'YYYYMMDD'),lpad(nextval('public.folio_solicitud_seq'::regclass)::text,6,'0'));

    insert into public.solicitudes(id_usuario,id_creado_por,id_tipo_solicitud,id_area,id_turno,id_jefe_usuario,folio,codigo_formato,numero_revision,fecha_revision_formato,contexto_snapshot)
    values(v_u.id,v_u.id,v_tipo.id,v_u.id_area,v_u.id_turno,v_jefe.id,v_folio,p_codigo_formato,p_numero_revision,p_fecha_revision_formato,
        jsonb_build_object('version',1,'canal','INTRANET','solicitante',jsonb_build_object('id',v_u.id,'numeroEmpleado',v_u.numero_empleado,'nombre',v_u.nombre_empleado,'correo',v_u.correo_microsoft,'puesto',v_u.puesto),'area',jsonb_build_object('id',v_a.id,'nombre',v_a.nombre_area),'turno',jsonb_build_object('id',v_t.id,'nombre',v_t.nombre_turno,'horasSemanales',v_t.horas_semanales),'responsable',jsonb_build_object('id',v_jefe.id,'nombre',v_jefe.nombre_empleado,'correo',v_jefe.correo_microsoft,'cargo',v_j.cargo_jefatura),'cliente',coalesce(p_contexto_cliente,'{}'::jsonb))) returning id into v_id;
    ...
    values(v_id,'SOLICITUD_CREADA','JEFE','PENDIENTE_JEFE',v_u.id,'APP',v_u.nombre_empleado,v_u.correo_microsoft,'Solicitud creada desde la intranet',jsonb_build_object('tipo',p_clave_tipo));
    return jsonb_build_object('idSolicitud',v_id,'folio',v_folio,'tipo',p_clave_tipo,'estado','PENDIENTE_JEFE');
